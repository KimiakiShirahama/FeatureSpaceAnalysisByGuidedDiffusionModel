...
class LatentDiffusion(DDPM):
    ...
    def check_image(self, target, reference, criterion, t_id, rec_id, bk_id, best_info, z_prev=None):
        if bk_id < 0:
            val = -1 * criterion(
                       target, reference, {"t_id":t_id, "rec_id":rec_id, "comment":best_info["target"]}
                   )
        else:
            val = -1 * criterion(
                       target, reference, {"t_id":t_id, "rec_id":rec_id, "bk_id":bk_id, "comment":best_info["target"]}
                   )
        if val.item() > best_info["val"]:
            target_str = "image" if z_prev is None else "z_prev"
            with open(criterion.module.log_filename, mode='a') as f:
                print(f'!!! Found a better {target_str} ({best_info["val"]} -> {val.item()})', file=f)
            best_info["t_id"], best_info["rec_id"], best_info["bk_id"] = t_id, rec_id, bk_id
            best_info["val"] = val.item()
            if z_prev is None:
                best_info["image"] = target.detach().clone()
            else:
                best_info["image"] = z_prev.detach().clone()
        return val
    ...
    def operation_diffusion(self, og_img, operated_image, cond, operation, return_first_stage_outputs=False,
        ...
        # og_img is just a dummy image to get the initial z
        if og_img != None:
            og_img = og_img.to(device)
        else:
            og_img = torch.rand(1, 3, operation.image_H, operation.image_W).to(device)
        ...
        for i in iterator:
            ...
            best_z_prev_info = {"target":"best z_prev", "t_id":-1, "rec_id":-1, "bk_id":-1, "val":-float("inf"), "image":None}

            sim_mavg = -1.0 * np.mean(sim_history)
            do_backward = True if sim_mavg < operation.avg_sim_threshold else False
            print(f"### {i}th timestep (sim_mavg: {sim_mavg}) ###")
            ...
            for j in range(num_steps):
                ...
                if operation.guidance_3:
                    ...
                    if operation.print:
                        if (i % operation.print_every == 0 or i == 999) and j == 0:
                            temp = (recons_image + 1) * 0.5
                            utils.save_image(temp, f'{operation.folder}/original_guidance_{i}_{j}.png')
                    ...
                    if operation.grad_clip_threshold > 0:
                        grad_norm_min = torch.mean(grad_norm) - operation.grad_clip_threshold * torch.std(grad_norm)
                        grad_norm_max = torch.mean(grad_norm) + operation.grad_clip_threshold * torch.std(grad_norm)
                        grad_norm = torch.clamp(grad_norm, grad_norm_min, grad_norm_max)
                    eps = model_output - operation.optim_guidance_3_wt * grad_norm
                    """
                    ...
                    # Get z_0 (z_recon) after modifying the predicted noise (i.e., eps)
                    ...
                    if operation.print:
                        if i % operation.print_every == 0 and j == 0:
                            temp = (recons_image + 1) * 0.5
                            utils.save_image(temp, f'{operation.folder}/guidance_3_{i}_{j}.png')
                    # Self-recurrence id is added by 0.5 to see that check_image is called after forward guidance
                    self.check_image(recons_image, operated_image, criterion, i, j+0.5, -1, best_image_info)
                ...
                # G2 guidance: Backward guidance
                # you have recons_image so optimize it
                if operation.guidance_2 and do_backward:

                    torch.set_grad_enabled(True)
                    z_recon = z_recon.detach().requires_grad_(True)

                    if operation.optimizer == 'Adam':
                        lr = operation.lr
                        optimizer = torch.optim.Adam([z_recon], lr=lr)

                    if operation.lr_scheduler == 'CosineAnnealingLR':
                        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, max_iters)

                    weights = torch.ones_like(z_recon).cuda()
                    ones = torch.ones_like(z_recon).cuda()
                    zeros = torch.zeros_like(z_recon).cuda()
                    ...
                    for k in range(max_iters):
                        ...
                        z_recon_before = torch.clone(z_recon.data)
                    ...
                    recons_image = self.decode_first_stage(z_recon)
                    self.check_image(recons_image, operated_image, criterion, i, j+0.9, max_iters, best_image_info)
                    if operation.print:
                        if i % operation.print_every == 0 and j == 0:
                            temp = (recons_image + 1) * 0.5
                ...
                if i != 0:
                    ...
                    coeff1 = 1 / torch.sqrt(1 - extract_into_tensor(self.betas, ts, z.shape))
                    coeff2 = 1 / extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, ts, z.shape)
                    coeff3 = extract_into_tensor(self.betas, ts, z.shape)
                ...
                # Evaluate the usefulness of the predicted z_prev
                if operation.select_z_prev and i != 0:
                    model_output_prev = self.apply_model(z_prev, ts_1, cond)
                    z_recon_prev = self.predict_start_from_noise(z_prev, t=ts_1, noise=model_output_prev)
                    recons_image_prev = self.decode_first_stage_with_grad(z_recon_prev)
                    self.check_image(
                        recons_image_prev, operated_image, criterion, i, j, -1, best_z_prev_info, z_prev
                    )

                ## take one more step
                if j != (num_steps - 1):  # Self-recurrence to recreate z from z_prev
                ...
                else:  # Go to the next timestep
                    if operation.select_z_prev and i != 0:
                        z = best_z_prev_info["image"]
                    else:
                        z = z_prev
        ...
        print(f"Output the image with the highest similarity")
        utils.save_image(
